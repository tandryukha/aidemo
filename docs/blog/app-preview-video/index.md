...
July 18, 2026 · Product Demo Videos · 8 min read · https://aidemo.top/blog/app-preview-video/
...
- Apple app previews run 15 to 30 seconds as an uploaded H.264 file (High Profile up to L4.0, 30 fps, under 500 MB); the modern iPhone class exports at 886 x 1920.
- Google Play takes no file: a listing's promo video is just a YouTube URL (one per localized listing), so Google inherits YouTube's encoding with no resolution or codec to hit.
- Apple guideline 2.3.4 allows only screen captures of the app itself, captured on device with no fingers or device frames, so stock and synthetic footage are out.
- Preview slots multiply: up to 3 per localization, so five languages is up to 15 App Store cuts, each 15 to 30 s and each invalidated when the UI it shows changes.
- The accepted 886 x 1920 is smaller than the 1320 x 2868 the phone paints, so hitting spec is an exact export, and a scripted capture re-renders instead of reshoots.

## Apple wants a file, Google wants a link
...
An App Store app preview is an uploaded video with narrow tolerances. Per Apple's App Store Connect reference, each preview runs 15 to 30 seconds, tops out at 500 MB, and ships as `.mov`, `.m4v`, or `.mp4` ([Apple, 2026](https://developer.apple.com/help/app-store-connect/reference/app-information/app-preview-specifications/)). The video track is H.264 High Profile up to Level 4.0 at a 10 to 12 Mbps target, or ProRes 422 HQ, and either way the frame rate is capped at 30 fps. You get up to three previews per localization, and the store lifts a poster frame from the 5-second mark by default.

The resolution is the part that trips people up, because it is fixed per device family and it is not your phone's native resolution. For the modern iPhone class — the 6.9-inch, 6.5-inch, 6.3-inch, and 6.1-inch displays — the accepted portrait resolution is 886 x 1920, or 1920 x 886 in landscape ([Apple, 2026](https://developer.apple.com/help/app-store-connect/reference/app-information/app-preview-specifications/)). That is smaller than the grid the phone actually paints: a 6.9-inch iPhone 16 Pro Max renders at 1320 x 2868 physical pixels ([Use Your Loaf, 2026](https://useyourloaf.com/blog/iphone-16-screen-sizes/)). So a raw screen capture off the device is the wrong size, and hitting the spec means exporting to an exact 886 x 1920 frame, not uploading whatever the recorder handed you.
...
Now multiply. Apple gives you up to three previews per localization ([Apple, 2026](https://developer.apple.com/help/app-store-connect/reference/app-information/app-preview-specifications/)). Localize the listing into five languages and you have authored, at the ceiling, fifteen App Store preview slots — fifteen separate 15-to-30-second cuts, each captured off the app, each landed on 886 x 1920. Add the Play promo video for each localized listing on top. That is the true surface area of "an app preview video," and none of it is a one-time cost, because guideline 2.3 asks you to keep it current and the footage rule means a single redesigned screen invalidates every clip that showed it.

A live-filmed preview has no cheap fix here. Because the footage came off a person driving the app in real time, one changed screen desyncs the cursor path, the timing, and the narration, so the honest response to a UI change is to shoot the whole cut again, times fifteen. This is the same [staleness that quietly dates every product demo](/blog/why-product-demos-go-stale), except the App Store enforces it as a metadata-accuracy rule rather than leaving it to your conscience.

## Hitting 886 by 1920 without shooting it twice

The way out is the one that makes any [product demo cheap to keep current](/blog/how-to-make-a-product-demo-video): stop performing the capture and start replaying it. If the path through the app is a written script — open this screen, tap this control, wait for that result — then the capture is deterministic, and [replaying the same flow yields the same footage every run](/blog/deterministic-browser-automation-for-video). A UI change becomes a diff to the script and a re-render, not a fifteen-cut reshoot, and the multi-cut math above stops compounding: the localized set becomes a [render matrix off one recording](/blog/multi-language-product-demo-videos) instead of fifteen separate shoots.
...
- [Apple — App preview specifications (App Store Connect Help)](https://developer.apple.com/help/app-store-connect/reference/app-information/app-preview-specifications/)
- [Apple — App Previews (Apple Developer)](https://developer.apple.com/app-store/app-previews/)
- [Apple — App Store Review Guidelines (2.3 Accurate Metadata)](https://developer.apple.com/app-store/review/guidelines/)
- [Google — Play Developer API, Listing resource (edits.listings)](https://developers.google.com/android-publisher/api-ref/rest/v3/edits.listings)
- [Use Your Loaf — iPhone 16 screen sizes](https://useyourloaf.com/blog/iphone-16-screen-sizes/)
