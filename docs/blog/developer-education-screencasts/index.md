...
July 18, 2026 · Video Documentation & Tutorials · 8 min read · https://aidemo.top/blog/developer-education-screencasts/
...
- On a 2560px screen recorded full-screen, a 14px editor font lands near 10.5px in a 1080p player and ~3.5px in a 640px embed. Record code at 18-22px and zoom to 60-80 columns.
- H.264's 4:2:0 subsampling discards 75% of color samples, so syntax highlighting blurs first. Pick a theme whose colors clear WCAG's 4.5:1 so they survive the codec.
- Record the terminal as text: asciinema stores a session as newline-delimited JSON viewers can copy-paste; vhs replays a .tape script to GIF or MP4 at a font size you set.
- Never read code aloud line by line. Show the one or two lines that change, zoomed, and narrate the decision, not the syntax.
- A code screencast breaks on two fronts: the UI and the code. A renamed flag makes the narration lie while the frame looks fine, so regenerate from the fixtures your tests use.

## When a generic recorder meets a page of code

Teaching a developer something means putting three surfaces on screen that most screen recorders were never tuned for: an editor packed with small colored text, a terminal that is really a stream of characters pretending to be a picture, and a browser showing the result. A recorder built for a webinar or a talking head treats all three as generic pixels, and all three fall apart in their own way. This is the format problem specific to a developer-education screencast, and it sits downstream of the pillar question of [when video earns a place in docs at all](/blog/video-documentation). Assume it has earned one. The next question is how to capture code so it still teaches after it has been through a codec.

## Why a codec eats your code before your reader does

Start with the compression, because it decides everything else. YouTube's own recommendation for a 1080p SDR upload at 30fps is H.264 in an MP4 at 8 Mbps ([Google, accessed July 2026](https://support.google.com/youtube/answer/1722171)). H.264 stores color at 4:2:0, which means the two chroma channels are "each subsampled at a factor of 2 both horizontally and vertically" ([Wikipedia, accessed July 2026](https://en.wikipedia.org/wiki/Chroma_subsampling)), discarding three quarters of the color samples and bleeding color across borders. Syntax highlighting is precisely the content that suffers: fine, saturated, high-contrast-edged text, where the purple that marks a keyword and the green that marks a string differ mostly in the channel the codec throws away first. Your theme looks crisp in the editor and muddy in the export, and nothing about your recorder caused it.
...
Two moves fix it, and both happen before you record. Raise the editor font to 18 to 22px, and zoom to the region so a 1080p frame carries 60 to 80 columns instead of 200. WCAG treats 18pt (about 24px) as "large text," the size at which a 3:1 contrast ratio is enough where smaller text needs 4.5:1 ([W3C, accessed July 2026](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html)). That threshold is a useful target for a screencast too, because it is roughly the size at which H.264 keeps a glyph's strokes intact. While you are there, choose a theme whose token colors clear 4.5:1 against the background; the low-contrast pairs a designer reaches for are the first casualties of 4:2:0.

## The terminal is text, so capture it as text

The terminal is the one surface you can escape the codec on entirely. asciinema records a session as newline-delimited JSON ([asciinema docs, accessed July 2026](https://docs.asciinema.org/manual/asciicast/v2/)), storing the characters and control sequences rather than pixels, so a reader can "pause the player and copy-paste the content you want. It's just text after all" ([asciinema, accessed July 2026](https://asciinema.org/)). vhs takes the scripted route: a `.tape` file it bills as "terminal GIFs as code," replaying typed commands and rendering to GIF, MP4, or WebM at a font size you `Set` yourself ([charmbracelet/vhs, accessed July 2026](https://raw.githubusercontent.com/charmbracelet/vhs/main/README.md)). The full tradeoff among asciinema, a raw recording, and tape-driven replay, scored on file size, editability, and whether viewers can copy the commands, is [a decision of its own](/blog/record-a-terminal-session).

For a screencast the binding constraint is compositional. An asciinema player is a web widget, not a video track, so it will not live inside the same MP4 as your browser footage. That leaves two honest paths: render the terminal with vhs and cut the clip into the timeline, where you control the font and it survives compression, or keep the browser as video and place the asciinema cast beside it as the copyable companion. What you should not do is point a [general-purpose recorder from a Camtasia-alternatives roundup](/blog/camtasia-alternatives) at a 12px terminal and hope the prompt reads; those tools capture a talking head well and a wall of monospace poorly.
...
API and SDK quickstarts rot fastest of all, which is the argument for making an [API-docs video regenerate from the same fixtures the docs are tested against](/blog/api-documentation-videos). The model that holds once you have more than a handful of tutorials is regeneration rather than re-recording ([keeping a tutorial library current](/blog/keeping-tutorial-libraries-current)), and a [coding agent can author the spec so the re-render rides the same release that changed the code](/blog/coding-agents-that-make-demo-videos). aidemo regenerates its browser layer from a committed storyboard instead of a fresh take, though the storyboard is agent-authored and edited as text, with no drag-and-drop timeline anywhere in the loop. Extend the same discipline to the other two surfaces: drive the terminal from a vhs tape and pull the code from the same fixtures your tests run, so all three layers rebuild together on the commit that changed them, or fail loudly instead of shipping a screencast that teaches a command that no longer exists.

## Sources

- [Google / YouTube Help — Recommended upload encoding settings (H.264, 8 Mbps for 1080p SDR)](https://support.google.com/youtube/answer/1722171)
- [Wikipedia — Chroma subsampling (4:2:0 subsamples Cb and Cr by a factor of 2 in both directions)](https://en.wikipedia.org/wiki/Chroma_subsampling)
- [W3C — Understanding SC 1.4.3 Contrast (Minimum): 4.5:1 normal, 3:1 large text, 18pt large-text threshold](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html)
- [asciinema — Record and share terminal sessions, the simple way (text, not blurry video; copy-paste from the player)](https://asciinema.org/)
- [asciinema docs — asciicast v2 file format (newline-delimited JSON, records terminal text)](https://docs.asciinema.org/manual/asciicast/v2/)
- [charmbracelet/vhs — Write terminal GIFs as code (.tape scripts, Set FontSize, GIF/MP4/WebM output)](https://raw.githubusercontent.com/charmbracelet/vhs/main/README.md)
