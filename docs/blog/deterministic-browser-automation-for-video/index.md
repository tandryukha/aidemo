...
July 18, 2026 · Demos as Code · 8 min read · https://aidemo.top/blog/deterministic-browser-automation-for-video/
...
- A test suite hides flakiness behind a rerun; Google measured ~1.5% of test runs and 16% of tests as flaky. A video has no rerun, so the flake is a frame you ship.
- The bar rises from 'eventually passes' to 'same pixels twice.' Wait on conditions (Playwright's stable check = same bounding box for 2 animation frames), never sleep(n).
- Freeze motion three ways: emulate prefers-reduced-motion, install a fake clock (Playwright v1.45 controls Date, timers, requestAnimationFrame), and drive kept motion over fixed frames.
- Pin the frame before capture: an explicit viewport, a fixed deviceScaleFactor (2 for crisp text), and await document.fonts.ready so a late font swap can't reflow the shot.
- Network variance can't be pinned from inside the browser: record against localhost or a seeded fixture with stubbed responses so pacing and on-screen numbers stay identical.

## A green test forgives what a rendered frame keeps

A test suite and a video renderer run the same browser through the same flow. They part ways on one word: rerun. When a test flakes, the runner retries it, and a pass on the second attempt is still a pass. That safety net is load-bearing at scale. Google, measuring its own corpus, reported that about 1.5% of all test runs report a flaky result, that almost 16% of its tests carry some level of flakiness, and that about 84% of the transitions it sees from pass to fail turn out to be flaky rather than real regressions ([Google Testing Blog, 2016](https://testing.googleblog.com/2016/05/flaky-tests-at-google-and-how-we.html)). A suite survives those numbers because the retry hides them.
...
That is why treating a demo as [a committed spec you can diff and rebuild](/blog/demos-as-code) only pays off if the rebuild is deterministic. A spec that renders a slightly different video each run is not a spec; it is a random seed. The rest of this is the checklist that closes the gap between the two bars, variable by variable: what drifts between runs, what that drift looks like on screen, and how to pin it.
...
- **Emulate reduced motion.** The `prefers-reduced-motion` media feature signals that the user wants an interface that "removes, reduces, or replaces motion-based animations" ([MDN, accessed July 2026](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion)); a well-built app already tones its animations down under it. Playwright can emulate it directly with `reducedMotion: 'reduce'`, so the app serves its calmer, less time-dependent motion for the capture ([Playwright, accessed July 2026](https://playwright.dev/docs/emulation)). That is free determinism for any motion the app agreed to drop.
- **Pin the clock.** For motion the app keeps, take away its clock. Playwright's clock API, added in v1.45, replaces the page's `Date`, `setTimeout`, `setInterval`, `requestAnimationFrame`, and `performance` with a fake one you control, so every timer- and animation-frame-driven effect advances by exactly the ticks you allow ([Playwright, accessed July 2026](https://playwright.dev/docs/api/class-clock)). Install it before you navigate and set it just behind the intended time, the docs note, so page-load timers still fire and nothing hangs. A pinned clock is also what makes a "3 minutes ago" label read the same in every render.
- **Drive the motion you keep.** For the one move you want on purpose, a reveal or a highlight, do not leave its duration to the browser. Compute it yourself over a fixed number of frames, so it is both smooth and identical every time.
...
The remedy is to record against a target you own: localhost, a seeded staging build, or a fixture with a pinned dataset, and to stub or mock the requests so responses come back instant and identical. This is the same reason [the agent seeds state before it writes the spec](/blog/coding-agents-that-make-demo-videos): the demo should depend on data it controls, not on whatever production happened to be serving. Filming a real service and hoping is how a golden render quietly [drifts out of true](/blog/why-product-demos-go-stale).
...
| Timers and clock | `Date`, `setTimeout`, rAF | spinner angle, "3 min ago" text | install a fake clock before navigating |
...
None of this is exotic. It is a fixed viewport, a locked pixel ratio, waits keyed to conditions rather than a countdown, motion under a frozen clock, and a network you own. An engine built for this bakes the list in: aidemo, which is ours, drives a real Chrome through a fixed action-spec with an injected cursor, pins the viewport and pixel ratio, and waits on conditions, so one storyboard renders the same frames on every run. It works only in the browser, its storyboards come from an agent rather than a drag-and-drop timeline, and it has no click-to-trim editor, which are the same constraints that make the take reproducible. Get the list right and the demo stops being a performance you hope repeats and becomes something [a CI job can rebuild on the commit that changed the UI](/blog/demo-videos-in-ci).

## Sources

- [Google Testing Blog — Flaky Tests at Google and How We Mitigate Them (1.5% of runs, 16% of tests, 84% of pass-to-fail transitions)](https://testing.googleblog.com/2016/05/flaky-tests-at-google-and-how-we.html)
- [Playwright — Auto-waiting and actionability checks (visible, stable, receives events, enabled)](https://playwright.dev/docs/actionability)
- [Playwright — Clock API (v1.45; mock Date, timers, requestAnimationFrame)](https://playwright.dev/docs/api/class-clock)
- [Playwright — Emulation (viewport, deviceScaleFactor, reducedMotion, colorScheme)](https://playwright.dev/docs/emulation)
- [Playwright — Videos (recordVideo on newContext, viewport-sized, saved on context close)](https://playwright.dev/docs/videos)
- [MDN — prefers-reduced-motion media feature](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion)
- [MDN — FontFaceSet.ready (promise resolves when fonts and layout are complete)](https://developer.mozilla.org/en-US/docs/Web/API/FontFaceSet/ready)
