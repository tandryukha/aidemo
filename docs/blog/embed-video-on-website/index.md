...
July 18, 2026 · Video Marketing for Software · 8 min read · https://aidemo.top/blog/embed-video-on-website/
...
- Three mechanisms: a self-hosted <video> (you own egress, no adaptive), a YouTube/Vimeo iframe (free but ~540 KB player plus tracking per load), or a streaming host (adaptive, clean player, metered).
- A cold YouTube embed loads a ~540 KB player on the page (Lighthouse); a facade ships ~3 KB and swaps in the real player on click, and lite-youtube-embed reports rendering ~224x faster.
- A 2-min 1080p demo at 10,000 views/mo costs ~$0 on YouTube (paid in page weight), ~$1.50-3 on Bunny or a commodity CDN, and ~$20 on Cloudflare Stream or Mux.
- Per-minute billing (Cloudflare, Mux) only beats per-GB billing (Bunny, self-host) above ~13 Mbps; for typical 1080p, per-GB is roughly seven times cheaper.
- Self-hosting a single MP4 gives every viewer one bitrate; adaptive 360p-1080p delivery is automatic on YouTube and streaming hosts but is an HLS pipeline you build yourself when self-hosting.
...
**Self-hosting** means you serve the encoded file from your own origin or CDN and point a `<video>` element at it. The browser fetches the file and plays it with native controls. Because no one format plays in every browser, you list a couple of fallbacks and, in MDN's words, "the browser will then use the first one it understands" ([MDN, 2026](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/video)); [which codec belongs in each source](/blog/best-video-format-for-screen-recording) is a separate call. Everything downstream of that tag is yours: the encoding, the poster, the caption track, and the bandwidth.
...
The cheapest option is not free; it just bills you in kilobytes instead of dollars. Chrome's Lighthouse audit for third-party facades puts a figure on it: for a YouTube embed, "the facade weighs 3 KB and the player weighing 540 KB is loaded on interaction" when you defer it, and on first paint when you don't ([Lighthouse, 2026](https://developer.chrome.com/docs/lighthouse/performance/third-party-facades)). Half a megabyte of someone else's JavaScript, executed before anything is clicked, for a video most visitors will never play.

The fix is the facade pattern: render a lightweight placeholder and swap in the real iframe only when the visitor interacts. Lighthouse spells out the sequence — "On load: Add facade to the page. On mouseover: The facade preconnects to third-party resources. On click: The facade replaces itself with the third-party product" ([Lighthouse, 2026](https://developer.chrome.com/docs/lighthouse/performance/third-party-facades)). Paul Irish's drop-in custom element for exactly this reports rendering "approximately 224× faster" than the native embed ([lite-youtube-embed, 2026](https://github.com/paulirish/lite-youtube-embed)).

The catch, in the audit's own words, is that a facade lacks "the full range of functionality of the actual embeds": no autoplay, no live badges, a click's worth of latency before playback starts. For a landing hero that is a fair trade, and the [LCP and poster mechanics that decide whether any embed wrecks first paint](/blog/demo-video-on-landing-page) are worked out in detail elsewhere in this cluster. The point here is narrower: the raw iframe is the single most expensive way to put a video on a page, and almost nobody who pastes it knows the price.
...
| --- | --- | --- | --- |
| Self-hosted, commodity CDN | GB delivered | ~$0.005-0.01/GB ([Bunny, 2026](https://bunny.net/stream/)) | ~$1.50-3, plus your own encoding |
| YouTube / Vimeo embed | nothing | $0 | $0 (paid in ~540 KB/load and tracking) |
| Cloudflare Stream | minutes delivered | $1 / 1,000 min, bandwidth included ([Cloudflare, 2026](https://developers.cloudflare.com/stream/pricing/)) | ~$20 |
| Mux | minutes delivered | $0.001/min at 1080p ([Mux, 2026](https://www.mux.com/pricing)) | ~$20 at list |
| Bunny Stream | GB delivered | from $0.005-0.01/GB ([Bunny, 2026](https://bunny.net/stream/)) | ~$1.50-3 |
...
- [Chrome / Lighthouse — Lazy load third-party resources with facades](https://developer.chrome.com/docs/lighthouse/performance/third-party-facades)
- [paulirish — lite-youtube-embed](https://github.com/paulirish/lite-youtube-embed)
- [Cloudflare Stream — Pricing](https://developers.cloudflare.com/stream/pricing/)
- [Cloudflare Stream — Overview (adaptive bitrate)](https://developers.cloudflare.com/stream/)
- [Mux — Pricing](https://www.mux.com/pricing)
- [Bunny — Stream pricing](https://bunny.net/stream/)
...
Yes, if you paste the stock iframe. Chrome's Lighthouse measures the YouTube player at about 540 KB loaded on the page ([Lighthouse, 2026](https://developer.chrome.com/docs/lighthouse/performance/third-party-facades)), executed on first paint whether or not anyone watches. The fix is a facade, a roughly 3 KB placeholder that swaps in the real player on click, which the lite-youtube-embed element reports rendering about 224 times faster.
